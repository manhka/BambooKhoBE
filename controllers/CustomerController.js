...
exports.createCustomer = async (req, res) => {
  ...
  if (!CustomerName || CustomerName.trim() === "") {
    return res
      .status(400)
      .json({ status: "error", message: "CustomerName_is_required" });
  }
  if (!Address || Address.trim() === "") {
    return res
      .status(400)
      .json({ status: "error", message: "Address_is_required" });
  }
  ...
  if (trimmedPhone === "") {
    return res
      .status(400)
      .json({ status: "error", message: "Phone_is_required" });
  }

  if (!phoneRegex.test(trimmedPhone)) {
    return res
      .status(400)
      .json({ status: "error", message: "Phone_format_invalid" });
  }
  ...
  try {
    ...
    if (phoneExists) {
      return res
        .status(400)
        .json({ status: "error", message: "Phone_already_exists" });
    }
    ...
    res
      .status(500)
      .json({ status: "error", message: "server_error", error: error.message });
  }
};
