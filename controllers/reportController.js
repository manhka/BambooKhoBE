...
exports.getQuarterReport = async (req, res) => {
  try {
    ...
    const imports = await ImportOrder.findAll({
      ...
      include: [
        { model: Supplier, attributes: ["SupplierID", "SupplierName"] },
        {
          model: ImportDetail,
          include: [{ model: Product, attributes: ["BarcodeProduct", "ProductName"] }],
        },
      ],
    });
...
exports.exportQuarterReport = async (req, res) => {
  try {
    ...
    worksheet.columns = [
      ...
      { header: "Ngày", key: "date", width: 15, style: { numFmt: 'dd/mm/yyyy' } },
      { header: "Sản Phẩm", key: "productName", width: 35 },
      { header: "Số Lượng", key: "quantity", width: 10, style: { numFmt: '#,##0' } },
      { header: "Đơn Giá", key: "unitPrice", width: 15, style: { numFmt: '#,##0 "VNĐ"' } },
      { header: "Thành Tiền", key: "total", width: 15, style: { numFmt: '#,##0 "VNĐ"' } },
      { header: "Đối Tác", key: "partner", width: 25 },
    ];

    const headerRow = worksheet.getRow(1);
    headerRow.font = { name: 'Arial', bold: true, size: 12, color: { argb: 'FFFFFFFF' } }; 
