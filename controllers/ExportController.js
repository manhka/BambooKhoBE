...
const { sequelize, ExportOrder, ExportDetail, Product, Customer, User, StaffActivity } = require("../models");
const { validateExportDetails, validateExportOrder } = require("../validators/ExportValidator");
...
exports.createExportOrder = async (req, res) => {
  ...
  try {
    ...
    if (!isValid) {
      await transaction.rollback();
      return res.status(400).json({ message: "Invalid export details.", errors });
    }
    ...
    for (const barcode in productUpdates) {
      const product = await Product.findByPk(barcode, { transaction, lock: transaction.LOCK.UPDATE });
      if (product) {
          if (product.NumberOfProduct < productUpdates[barcode]) {
             throw new Error(`Insufficient stock for ${barcode} during final update check.`);
          }
    ...
    res.status(201).json({ message: "Export order created successfully!", exportOrder });

  } catch (error) {
    await transaction.rollback();
    console.error("Create Export Error:", error);
    res.status(500).json({ message: "Server error during export creation.", error: error.message });
  }
};
...
exports.getExportHistory = async (req, res) => {
    try {
        ...
        const includeOptions = [
            { model: ExportDetail, as: 'ExportDetails', include: [{ model: Product, as: 'Product', attributes: ['ProductName'] }] },
            { model: Customer, as: 'Customer', attributes: ['CustomerName'] },
            { model: User, as: 'User', attributes: ['Username'] }
        ];

        if (fromDate && toDate) {
            orderWhereClause.ExportDate = { [Op.between]: [new Date(fromDate), new Date(toDate)] };
        } else if (fromDate) {
        ...
        if (barcode) {
             const relevantExportIDs = await ExportDetail.findAll({
                 attributes: ['ExportID'], where: { BarcodeProduct: barcode }, group: ['ExportID']
             }).then(details => details.map(d => d.ExportID));
             if (relevantExportIDs.length === 0) {
                 return res.status(200).json({ message: "Export history retrieved successfully", count: 0, orders: [] });
             }
        ...
        const orders = await ExportOrder.findAll({
            ...
            order: [['ExportDate', 'DESC'], ['ExportID', 'DESC']]
        });

        res.status(200).json({ message: "Export history retrieved successfully", count: orders.length, orders });
    ...
    try {
        ...
        const order = await ExportOrder.findByPk(id, {
             include: [
                 ...
                 { model: Customer, as: 'Customer', attributes: ['CustomerName', 'Address', 'Phone'] },
...
exports.exportOrderToExcel = async (req, res) => {
    try {
        ...
        // Thông tin chung
        worksheet.addRow(['','Mã Phiếu Xuất:', order.ExportID, '', 'Ngày Xuất:', new Date(order.ExportDate).toLocaleDateString('vi-VN')]);
        ...
        worksheet.addRow(['','Khách Hàng:', order.Customer?.CustomerName || 'N/A', '', 'Người Tạo:', order.User?.Username || 'N/A']);
        worksheet.getRow(4).getCell('C').font = { bold: true };
        worksheet.getRow(4).getCell('F').font = { bold: true };
        worksheet.addRow([]);

        // Header bảng chi tiết 
        worksheet.addRow(['STT', 'Mã Sản Phẩm', 'Tên Sản Phẩm', 'Số Lượng', 'Đơn Giá', 'Bảo Hành (Tháng)', 'Thành Tiền']);
        ...
        order.ExportDetails.forEach((detail, index) => {
            const warrantyText = detail.WarrantyTime !== null ? `${detail.WarrantyTime} tháng` : 'Không BH';
